...
// 1. SPLASH SCREEN & AUTO-LOGIN ON LOAD
document.addEventListener("DOMContentLoaded", () => {
    const savedUser = localStorage.getItem("transmissions_user");
...
function closeModalAction(isConfirm) {
...
// CLEANED: Databases are now empty
let notesDB = { admin: "", kate: "" };
let currentNoteView = ""; 
let unreadNotes = { admin: false, kate: false }; 

// CLEANED: Placeholder avatars until Firebase is connected
const avatars = {
...
function attemptLogin() {
    ...
    if ((user === 'admin' || user === 'kate') && pass === '1234') {
        localStorage.setItem("transmissions_user", user); 
        executeLogin(user);
    } else {
...
async function logoutApp() {
    ...
    if (confirm) {
        localStorage.removeItem("transmissions_user");
        currentUser = "";
...
// 4. REFRESH APP
function refreshFeed(btnElement) {
...
function updateNoteUI() {
    ...
    if (!textToShow || textToShow === "") {
        noteEl.innerText = currentNoteView === currentUser ? "Tap to add note" : "No note yet";
    ...
    if (unreadNotes[currentUser] && currentNoteView === currentUser) { dotEl.style.display = 'block'; } 
    else { dotEl.style.display = 'none'; }
}

function toggleNoteView() {
    currentNoteView = currentNoteView === 'admin' ? 'kate' : 'admin';
    if (currentNoteView !== currentUser) { unreadNotes[currentUser] = false; }
    updateNoteUI();
}

async function editQuickNote() {
    if (currentNoteView !== currentUser) { 
    ...
    notesDB[currentUser] = newNote.trim();
    updateNoteUI();
}

async function deletePost(btnElement) {
    const confirm = await showCustomModal({
...
async function promptReaction(btnElement) {
...
function activateDockIcon(event) {
...
// 10. PREMIUM HIGHLIGHTS (STORIES) LOGIC - CLEANED FOR PRODUCTION
...
function showCurrentStory() {
    ...
    clearTimeout(storyTimer); storyTimer = setTimeout(nextStory, 5000); 
}

function nextStory() { if (currentStoryIndex < storiesData.length - 1) { currentStoryIndex++; showCurrentStory(); } else { closeStory(); } }
function prevStory() { if (currentStoryIndex > 0) { currentStoryIndex--; showCurrentStory(); } else { showCurrentStory(); } }
