...
// THE REAL FIREBASE SECURITY LOCK
...
window.editQuickNote = async function() {
    ...
    // Figure out if the current user is playing the role of Admin or Kate
    const myRole = (currentUser === MY_ADMIN_EMAIL) ? 'admin' : 'kate';
    
    // If you are looking at the OTHER person's note, block the edit
    if (currentNoteView !== myRole) { 
...
// 8. UPLOAD MODAL LOGIC WITH FIREBASE CONNECTION
...
window.submitMockUpload = async function() { 
    ...
    if (!caption) {
        showCustomModal({ title: "Transmission Failed", message: "Please write a message first!" });
        return;
    }

    try {
        // Send the text data to the Firebase "transmissions" vault
        ...
        await showCustomModal({ title: "Transmission Set", message: "Text data successfully saved to the server vault!" });
        closeUpload(); 
        document.getElementById('uploadCaption').value = ""; // Clear the box
        
    } catch (error) {
        showCustomModal({ title: "Access Denied", message: "The security vault rejected your upload: " + error.message });
    }
}
